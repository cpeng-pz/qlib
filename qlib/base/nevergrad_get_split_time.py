...
# nevergrad细粒度可以更改
SPLIT_N = 20
def nevergrad_get_split_time(ary, indices_or_sections, df, axis=0):
    ...
    # 打开文件以写入模式（'w'表示写入）
    with open(r"C:\Users\hehailing\Desktop\实验\sub_arys.txt", "a") as file:
        # 使用循环遍历列表中的元素，并逐行写入文件
        for item in sub_arys:
            file.write(str(item) + "\n")
        # 打开文件以写入模式（'w'表示写入）
    with open(r"C:\Users\hehailing\Desktop\实验\loss.txt", "a") as file:
        file.write(str(indices_or_sections) + "\n")
        # 直接将浮点数写入文件
        file.write(str(loss) + "\n")

    with open(r"C:\Users\hehailing\Desktop\实验\run_time.txt", "a") as file:
        file.write(str(indices_or_sections)+ "\n")
        # 使用循环遍历列表中的元素，并逐行写入文件
